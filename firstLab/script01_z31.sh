...
do
        ...
        then
                echo "Задание"
                echo "31. Обработать список входных параметров, скрипт должен
	       	различать 7 типов параметров и вывадить сообщение о том, какой
	       	параметр был получен и какое знаечение передается после параметра" 
                echo ""
                echo ""
                break
        fi
